<script setup>
    ...
    const agregar = (roomID, serviceID) => {
        Inertia.post(route('rooms.services.attach', {room: roomID, service: serviceID}), {
            onSuccess: () => {
                console.log('Borrado');
            },
        });
    ...
    const borrar = (roomID, serviceID) => {
        Inertia.post(route('rooms.services.detach', {room: roomID, service: serviceID}), {
            onSuccess: () => {
                console.log('Borrado');
            },
        });
...
<template>
    ...
    <AppLayout>
        ...
        <Forms>
            <h3 v-if="exclusive_services.length > 0" class="text-center text-xl mb-2">Servicios exclusivos vinculados la habitación N°{{ room.numero }}</h3>
            <div class="grid grid-cols-4 gap-2">
                <button v-for="service in services.filter(service => exclusive_services.includes(service.id))" :key="service.id" @click="borrar(room.id, service.id)" class="w-full bg-rojo border border-rojo-dark text-crema rounded-lg p-3 mb-2">
                    <div class="flex items-center justify-start">
                        ...
                        <span class="ml-2 text-base">{{ service.nombre }} - {{ service.tipo }}</span>
                    </div>
                </button>
            </div>
            <h3 v-if="exclusive_services.length < services.length" class="text-center text-xl mb-2">Servicios exclusivos no vinculados la habitación N°{{ room.numero }}</h3>
            <div class="grid grid-cols-4 gap-2">
                <button v-for="service in services.filter(service => !exclusive_services.includes(service.id))" :key="service.id" @click="agregar(room.id, service.id)" class="w-full bg-rojo-dark border border-rojo text-crema-dark rounded-lg p-3 mb-2">
                    <div class="flex items-center justify-start">
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" class="w-6 h-6">
            ...
            <div class="flex items-center justify-start">
                ...
                <PrimaryLink class="ms-4" :href="route('admin.rooms.show', room)">
                    Regresar
                </PrimaryLink>
            </div>
        </Forms>
