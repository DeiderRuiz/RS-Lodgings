<script setup>
    ...
    const agregar = (dealID, serviceID) => {
        Inertia.post(route('deal.services.attach', {deal: dealID, service: serviceID}), {
            onSuccess: () => {
                console.log('Borrado');
            },
        });
    ...
    const borrar = (dealID, serviceID) => {
        Inertia.post(route('deal.services.detach', {deal: dealID, service: serviceID}), {
            onSuccess: () => {
                console.log('Borrado');
            },
        });
...
<template>
    ...
    <AppLayout>
        ...
        <Forms>
            <h3 class="text-center text-xl mb-2">Servicios incluidos en la oferta {{ deal.nombre }}</h3>
            <div class="grid grid-cols-4 gap-2">
                <button v-for="service in services.filter(service => attached_services.includes(service.id))" :key="service.id" @click="borrar(deal.id, service.id)" class="w-full bg-rojo border border-rojo-dark text-crema rounded-lg p-3 mb-2">
                    <div class="flex items-center justify-start">
                        ...
                        <span class="ml-2 text-base">Servicio {{ service.nombre }} - {{ service.tipo }}</span>
                    </div>
                </button>
            </div>
            <h3 class="text-center text-xl mb-2">Servicios no incluidos en la oferta {{ deal.nombre }}</h3>
            <div class="grid grid-cols-4 gap-2">
                <button v-for="service in services.filter(service => !attached_services.includes(service.id))" :key="service.id" @click="agregar(deal.id, service.id)" class="w-full bg-rojo-dark border border-rojo text-crema-dark rounded-lg p-3 mb-2">
                    <div class="flex items-center justify-start">
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" class="w-6 h-6">
