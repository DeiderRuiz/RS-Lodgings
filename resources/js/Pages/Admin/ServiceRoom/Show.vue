<script setup>
    ...
    const agregar = (serviceID, roomID) => {
        Inertia.post(route('services.rooms.attach', {service: serviceID, room: roomID}), {
            onSuccess: () => {
                console.log('Borrado');
            },
        });
    ...
    const borrar = (serviceID, roomID) => {
        Inertia.post(route('services.rooms.detach', {service: serviceID, room: roomID}), {
            onSuccess: () => {
                console.log('Borrado');
            },
        });
...
<template>
    ...
    <AppLayout>
        ...
        <Forms>
            <h3 class="text-center text-xl mb-2">Habitaciones vinculadas al servicio {{ service.nombre }} ({{ service.tipo }})</h3>
            <div class="grid grid-cols-6 gap-2">
                <button v-for="room in rooms.filter(room => attached_rooms.includes(room.id))" :key="room.id" @click="borrar(service.id, room.id)" class="w-full bg-rojo border border-rojo-dark text-crema rounded-lg p-3 mb-2">
            ...
            <h3 class="text-center text-xl mb-2">Habitaciones no vinculadas al servicio {{ service.nombre }} ({{ service.tipo }})</h3>
            <button class="grid grid-cols-6 gap-2">
                <div v-for="room in rooms.filter(room => !attached_rooms.includes(room.id))" :key="room.id" @click="agregar(service.id, room.id)" class="w-full bg-rojo-dark border border-rojo text-crema-dark rounded-lg p-3 mb-2">
                    <div class="flex items-center justify-start">
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" class="w-6 h-6">
            ...
            <div class="flex items-center justify-start">
                ...
                <PrimaryLink class="ms-4" :href="route('admin.services.show', service)">
                    Regresar
                </PrimaryLink>
            </div>
        </Forms>
