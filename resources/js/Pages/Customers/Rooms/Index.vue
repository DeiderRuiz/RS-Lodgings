      ...
      <div class="grid grid-cols-4 gap-4">
        ...
        <div v-for="room in rooms" :key="room.id" class="max-w-sm bg-crema border border-crema-dark rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
          ...
          <div class="p-5 text-center">
            ...
            <PrimaryLink v-if="room.disponibilidad" :href="route('customer.reservations.create', room)" class="flex justify-center w-full mb-5">
              Ir a reservar
            </PrimaryLink>
            <!-- Dehabilita el botón de reservar si la habpitación no está disponible -->
            <PrimaryLink v-else :disabled="Boolean(!room.disponibilidad)" :href="route('customer.reservations.create', room)" class="flex justify-center bg-rojo-dark hover:bg-rojo w-full mb-5">
              Reservado
            </PrimaryLink>
            <!-- Detalles de la habitación -->
            <SecondaryLink :href="route('customer.rooms.show', room)" class="flex justify-center w-full">
              Ver Detalles                         
            </SecondaryLink>
          </div>
        </div>
